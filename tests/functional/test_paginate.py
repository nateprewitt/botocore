...
class TestRDSPagination(BaseSessionTest):
    ...
    def test_can_specify_zero_marker(self):
        ...
        # parameters.
        self.stubber.add_response(function_name, service_response, expected_params)
...
class TestAutoscalingPagination(BaseSessionTest):
    ...
    def _setup_scaling_pagination(self, page_size=200, max_items=100, total_items=600):
        """
        ...
        for i in range(num_pages):
            ...
            for _ in range(requests_per_page - 1):
                ...
                self.stubber.add_response("describe_scaling_activities", page.copy())
...
class TestCloudwatchLogsPagination(BaseSessionTest):
    ...
    def test_token_with_triple_underscores(self):
        response = {
            ...
            "searchedLogStreams": [
                {"logStreamName": "foobar", "searchedCompletely": False,}
            ],
        }
        group_name = "foo"
        token = "foo___bar"
        ...
        pages = paginator.paginate(
            PaginationConfig={"MaxItems": 1, "StartingToken": token,},
            logGroupName=group_name,
        )
        result = pages.build_full_result()
