...
def test_lint_pagination_configs():
    ...
    for service_name in services:
        ...
        for op_name, single_config in page_config["pagination"].items():
            yield (_lint_single_paginator, op_name, single_config, service_model)


def _lint_single_paginator(operation_name, page_config, service_model):
    ...
    for key in page_config:
        if key not in KNOWN_PAGE_KEYS:
            raise AssertionError(
                "Unknown key '%s' in pagination config: %s" % (key, page_config)
            )
    ...
    if "limit_key" in page_config:
        ...
        if limit_key not in valid_input_names:
            raise AssertionError(
                ...
                "%s" % (limit_key, operation_name, ", ".join(list(valid_input_names)))
            )
...
def _validate_output_keys_match(operation_name, page_config, service_model):
    ...
    if output_members:
        ...
        raise AssertionError(
            ...
            % (operation_name, service_model.service_name, ", ".join(output_members))
        )
