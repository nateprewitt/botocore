...
class TestStubber(unittest.TestCase):
    def setUp(self):
        ...
        config = botocore.config.Config(
            signature_version=botocore.UNSIGNED, s3={"addressing_style": "path"}
        )
    ...
    def test_activated_stubber_errors_with_no_registered_stubs(self):
        ...
        with self.assertRaisesRegexp(UnStubbedResponseError, "Unexpected API Call"):
    ...
    def test_expected_params_fail(self):
        ...
        with self.assertRaisesRegexp(StubResponseError, "{'Bucket': 'bar'},\n"):
            self.client.list_objects(Bucket="foo")
    ...
    def test_mixed_any_and_concrete_params(self):
        ...
        self.stubber.add_response("head_object", service_response, expected_params)
        self.stubber.add_response("head_object", service_response, expected_params)
        ...
        try:
            with self.stubber:
                self.client.put_object(
                    Bucket="foo", Key="bar.txt", Metadata={"MyMeta": "Foo",}
                )
                self.client.put_object(
                    Bucket="foo", Key="bar.txt", Metadata={"MyMeta": "Bar",}
                )
        except StubAssertionError:
            self.fail("stub.ANY failed to ignore nested parameter for validation.")
    ...
    def test_many_expected_params(self):
        ...
        try:
            ...
            self.fail("Stubber inappropriately raised error for same parameters.")
    ...
    def test_no_stub_for_presign_url(self):
        try:
            with self.stubber:
                ...
                self.assertEqual(url, "https://s3.amazonaws.com/mybucket/mykey")
    ...
    def test_can_stub_with_presign_url_mixed_in(self):
        ...
        self.stubber.add_response("list_objects", desired_response, expected_params)
        with self.stubber:
            url = self.client.generate_presigned_url(
                ClientMethod="get_object",
                Params={"Bucket": "myotherbucket", "Key": "myotherkey"},
            )
            self.assertEqual(url, "https://s3.amazonaws.com/myotherbucket/myotherkey")
    ...
    def test_parse_get_bucket_location(self):
        ...
        self.stubber.add_client_error("get_bucket_location", error_code, error_message)
        self.stubber.activate()
