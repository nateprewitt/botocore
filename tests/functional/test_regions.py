...
def test_known_endpoints():
    ...
    # logic evolves.
    resolver = _get_patched_session()._get_internal_component("endpoint_resolver")
    for region_name, service_dict in KNOWN_REGIONS.items():
        for service_name, endpoint in service_dict.items():
            yield (
                _test_single_service_region,
                service_name,
                region_name,
                endpoint,
                resolver,
...
def _test_single_service_region(service_name, region_name, expected_endpoint, resolver):
...
def test_known_endpoints():
    ...
    for service_name, endpoint in KNOWN_AWS_PARTITION_WIDE.items():
        yield (
            _test_single_service_partition_endpoint,
            service_name,
            endpoint,
            resolver,
        )
...
def _test_single_service_partition_endpoint(service_name, expected_endpoint, resolver):
...
class TestEndpointResolution(BaseSessionTest):
    ...
    def test_regionalized_client_with_unknown_region(self):
        ...
        self.assertEquals(stubber.requests[0].url, "https://s3.not-real.amazonaws.com/")
