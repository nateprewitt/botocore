...
class TestIdempotencyToken(unittest.TestCase):
    def setUp(self):
        ...
        self.client.meta.events.register_first(
            "before-call.*.*", self.collect_params, unique_id="TestIdempotencyToken"
        )
    ...
    def test_provided_idempotency_token(self):
        expected_params = {
            "PurchaseRequests": [{"PurchaseToken": "foo", "InstanceCount": 123}],
            "ClientToken": ANY,
        }
    ...
    def test_copy_snapshot_injects_presigned_url(self):
        ...
        with self.http_stubber:
            result = self.client.copy_snapshot(
                SourceRegion="us-west-2", SourceSnapshotId=self.snapshot_id,
            )
