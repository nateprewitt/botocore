...
class BaseRetryTest(BaseSessionTest):
    ...
    def assert_will_retry_n_times(self, client, num_retries, status=500, body=b"{}"):
...
class TestLegacyRetry(BaseRetryTest):
    ...
    def test_setting_max_attempts_does_not_set_for_other_clients(self):
        ...
        self.session.create_client(
            "codecommit", self.region, config=Config(retries={"max_attempts": 1})
        )
    ...
    def test_set_max_attempts_on_session(self):
        self.session.set_default_client_config(Config(retries={"max_attempts": 1}))
    ...
    def test_can_clobber_max_attempts_on_session(self):
        ...
        client = self.session.create_client(
            "codecommit", self.region, config=Config(retries={"max_attempts": 0})
        )
        with self.assert_will_retry_n_times(client, 0):
            client.list_repositories()
...
class TestRetriesV2(BaseRetryTest):
    def create_client_with_retry_mode(self, service, retry_mode, max_attempts=None):
    ...
    def test_standard_mode_has_default_3_retries(self):
        client = self.create_client_with_retry_mode("dynamodb", retry_mode="standard")
    ...
    def test_standard_mode_retry_throttling_error(self):
        ...
        with self.assert_will_retry_n_times(client, 2, status=400, body=error_body):
            client.list_tables()
    ...
    def test_adaptive_mode_still_retries_errors(self):
        ...
        client = self.create_client_with_retry_mode("dynamodb", retry_mode="adaptive")
        with self.assert_will_retry_n_times(client, 2):
            client.list_tables()
