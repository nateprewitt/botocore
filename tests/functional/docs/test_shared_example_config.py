...
def _lint_single_example(operation_name, example_config, service_model):
    ...
    assert_valid_values(service_model.service_name, operation_model, example_config)
...
def assert_valid_values(service_name, operation_model, example_config):
    ...
    if input_shape is None and example_input:
        raise AssertionError(
            ...
            "shape is defined." % (operation_model.name, service_name, example_id)
        )
    ...
    try:
        if example_input is not None and input_shape is not None:
            _assert_valid_values(input_shape, example_input, [input_shape.name])

        if example_output is not None and output_shape is not None:
            _assert_valid_values(output_shape, example_output, [output_shape.name])
...
def _assert_valid_structure_values(shape, example_dict, path):
    invalid_members = [k for k in example_dict.keys() if k not in shape.members]
    if invalid_members:
...
def _assert_valid_timestamp(timestamp, path):
    try:
        ...
        dotted_path = ".".join(path)
        raise AssertionError(
            "Failed to parse timestamp %s for %s: %s" % (timestamp, dotted_path, e)
        )
