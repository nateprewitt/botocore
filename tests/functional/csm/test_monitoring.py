...
def _configured_session(case_configuration, listener_port):
    ...
    with temporary_file("w") as f:
        _setup_shared_config(f, case_configuration["sharedConfigFile"], environ)
...
def _run_test_case(case):
    with MonitoringListener() as listener:
        with _configured_session(case["configuration"], listener.port) as session:
            for api_call in case["apiCalls"]:
                _make_api_call(session, api_call)
    assert_equal(listener.received_events, case["expectedMonitoringEvents"])
