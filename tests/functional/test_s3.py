...
class TestS3ClientConfigResolution(BaseS3ClientConfigurationTest):
    ...
    def test_use_arn_region(self):
        ...
        client = self.create_s3_client()
        self.assertEqual(client.meta.config.s3, {"use_arn_region": True,})
    ...
    def test_use_arn_region_env_var_overrides_config_var(self):
        ...
        with temporary_file("w") as f:
            self.set_config_file(f, "[default]\n" "s3 = \n" "    use_arn_region = true")
            client = self.create_s3_client()
        self.assertEqual(client.meta.config.s3, {"use_arn_region": False,})
    ...
    def test_us_east_1_regional_env_var(self):
        ...
        client = self.create_s3_client()
        self.assertEqual(
            client.meta.config.s3, {"us_east_1_regional_endpoint": "regional",}
        )
    ...
    def test_client_config_us_east_1_regional_overrides_env_var(self):
        ...
        self.assertEqual(
            client.meta.config.s3, {"us_east_1_regional_endpoint": "legacy",}
        )
...
class TestS3Copy(BaseS3OperationTest):
    ...
    def test_s3_copy_object_with_empty_response(self):
        ...
        response = self.client.copy_object(
            Bucket="bucket", CopySource="other-bucket/test.txt", Key="test.txt",
        )
...
class TestAccesspointArn(BaseS3ClientConfigurationTest):
    ...
    def test_missing_region_in_arn(self):
        accesspoint_arn = "arn:aws:s3::123456789012:accesspoint:myendpoint"
    ...
    def test_accesspoint_includes_dot(self):
        accesspoint_arn = "arn:aws:s3:us-west-2:123456789012:accesspoint:my.endpoint"
        with self.assertRaises(botocore.exceptions.ParamValidationError):
            self.client.list_objects(Bucket=accesspoint_arn)
    ...
    def test_endpoint_redirection(self):
        ...
        for region in regions:
            ...
            self.client.write_get_object_response(
                RequestRoute="endpoint-io.a1c1d5c7", RequestToken="SecretToken",
            )
            request = self.http_stubber.requests[0]
            self.assert_signing_name(request, "s3-object-lambda")
...
def test_checksums_included_in_expected_operations():
    ...
    yield t.case("put_bucket_tagging", {"Bucket": "foo", "Tagging": {"TagSet": []}})
    yield t.case(
    ...
    yield t.case("put_bucket_acl", {"Bucket": "foo", "AccessControlPolicy": {}})
    yield t.case("put_bucket_logging", {"Bucket": "foo", "BucketLoggingStatus": {}})
    yield t.case(
        "put_bucket_notification", {"Bucket": "foo", "NotificationConfiguration": {}}
    )
    yield t.case("put_bucket_policy", {"Bucket": "foo", "Policy": "<bucket-policy>"})
    yield t.case(
    ...
    yield t.case("put_bucket_website", {"Bucket": "foo", "WebsiteConfiguration": {}})
    yield t.case(
...
def test_correct_url_used_for_s3():
    ...
    yield t.case(
        ...
        bucket=accesspoint_arn_gov,
        key="key",
        s3_config={"use_arn_region": True, "use_dualstack_endpoint": True,},
        expected_url=(
    ...
    def test_escape_keys_in_xml_delete_objects(self):
        ...
        with self.http_stubber:
            response = self.client.delete_objects(
                Bucket="mybucket", Delete={"Objects": [{"Key": "some\r\n\rkey"}]},
            )
        request = self.http_stubber.requests[0]
        self.assertNotIn(b"\r\n\r", request.body)
        self.assertIn(b"&#xD;&#xA;&#xD;", request.body)
