...
class BaseSessionTest(unittest.TestCase):
    def setUp(self):
        ...
        config_path = os.path.join(os.path.dirname(__file__), "cfg", "foo_config")
...
class SessionTest(BaseSessionTest):
    ...
    def test_supports_multiple_env_vars_for_single_logical_name(self):
        env_vars = {
            "profile": (None, ["BAR_DEFAULT_PROFILE", "BAR_PROFILE"], None, None),
        }
    ...
    def test_profile(self):
        ...
        self.assertEqual(self.session.get_config_variable("region"), "us-west-11")
    ...
    def test_type_conversions_occur_when_specified(self):
        ...
        # of int().
        self.env_vars["metadata_service_timeout"] = (None, "FOO_TIMEOUT", None, int)
        # Environment variables are always strings.
        self.environ["FOO_TIMEOUT"] = "10"
        session = create_session(session_vars=self.env_vars)
        # But we should type convert this to a string.
        self.assertEqual(session.get_config_variable("metadata_service_timeout"), 10)
    ...
    def test_path_not_in_available_profiles(self):
        with temporary_file("w") as f:
            ...
            self.assertEqual(set(profiles), set(["foo", "default", "newprofile"]))
    ...
    def test_emitter_can_be_passed_in(self):
        ...
        session = create_session(session_vars=self.env_vars, event_hooks=events)
        calls = []
        handler = lambda **kwargs: calls.append(kwargs)
...
class TestSessionConfigurationVars(BaseSessionTest):
    def test_per_session_config_vars(self):
        self.session.session_var_map["foobar"] = (None, "FOOBAR", "default", None)
        # Default value.
        ...
        self.assertEqual(self.session.get_config_variable("foobar"), "session-instance")

        # Can disable this check via the ``methods`` arg.
        del self.environ["FOOBAR"]
        self.assertEqual(
            self.session.get_config_variable("foobar", methods=("env", "config")),
            "default",
        )
    ...
    def test_can_get_session_vars_info_from_default_session(self):
        ...
        self.assertEqual(
            ...
            ("metadata_service_timeout", "AWS_METADATA_SERVICE_TIMEOUT", 1, int),
        )
        self.assertEqual(
...
class TestSessionPartitionFiles(BaseSessionTest):
    def test_lists_partitions_on_disk(self):
        ...
        self.session._register_internal_component("endpoint_resolver", mock_resolver)
    ...
    def test_proxies_list_endpoints_to_resolver(self):
        ...
        self.session._register_internal_component("endpoint_resolver", resolver)
...
class TestConfigLoaderObject(BaseSessionTest):
    def test_config_loader_delegation(self):
        session = create_session(session_vars=self.env_vars, profile="credfile-profile")
        with temporary_file("w") as f:
...
class TestGetServiceModel(BaseSessionTest):
    def test_get_service_model(self):
        loader = mock.Mock()
        loader.load_service_model.return_value = {"metadata": {"serviceId": "foo"}}
...
class TestGetPaginatorModel(BaseSessionTest):
    def test_get_paginator_model(self):
        ...
        loader.load_service_model.assert_called_with("foo", "paginators-1", None)
...
class TestCreateClient(BaseSessionTest):
    ...
    def test_cred_provider_called_when_partial_creds_provided(self):
        with self.assertRaises(botocore.exceptions.PartialCredentialsError):
            self.session.create_client(
                "sts", "us-west-2", aws_access_key_id="foo", aws_secret_access_key=None
            )
        with self.assertRaises(botocore.exceptions.PartialCredentialsError):
            self.session.create_client(
                "sts", "us-west-2", aws_access_key_id=None, aws_secret_access_key="foo",
            )
    ...
    def test_create_client_with_merging_client_configs(self, client_creator):
        ...
        used_client_config = client_creator.return_value.create_client.call_args[1][
            "client_config"
        ]
    ...
    def test_create_client_with_verify_param(self, client_creator):
        self.session.create_client("ec2", "us-west-2", verify="verify-certs.pem")
        call_kwargs = client_creator.return_value.create_client.call_args[1]
    ...
    def test_can_specify_multiple_versions_from_config(self, client_creator):
        ...
        with temporary_file("w") as f:
            ...
            call_kwargs = client_creator.return_value.create_client.call_args[1]
            self.assertEqual(call_kwargs["api_version"], second_config_api_version)
...
class TestSessionComponent(BaseSessionTest):
    def test_internal_component(self):
        ...
        self.assertIs(self.session._get_internal_component("internal"), component)
        with self.assertRaises(ValueError):
            self.session.get_component("internal")

    def test_internal_endpoint_resolver_is_same_as_deprecated_public(self):
        endpoint_resolver = self.session._get_internal_component("endpoint_resolver")
        self.assertIs(
    ...
    def test_internal_exceptions_factory_is_same_as_deprecated_public(self):
        exceptions_factory = self.session._get_internal_component("exceptions_factory")
        self.assertIs(
            self.session.get_component("exceptions_factory"), exceptions_factory
        )
