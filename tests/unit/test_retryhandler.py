...
class TestRetryCheckers(unittest.TestCase):
    ...
    def test_max_attempts(self):
        ...
        self.assert_should_be_retried((HTTP_500_RESPONSE, response), attempt_number=1)
        self.assert_should_be_retried((HTTP_500_RESPONSE, {}), attempt_number=2)
    ...
    def test_max_attempts_successful(self):
        ...
        self.assert_should_be_retried((HTTP_500_RESPONSE, {}), attempt_number=1)
        # The second retry is successful.
        self.assert_should_not_be_retried((HTTP_200_RESPONSE, {}), attempt_number=2)

        # But now we can reuse this object.
        self.assert_should_be_retried((HTTP_500_RESPONSE, {}), attempt_number=1)
        self.assert_should_be_retried((HTTP_500_RESPONSE, {}), attempt_number=2)
        self.assert_should_not_be_retried((HTTP_500_RESPONSE, {}), attempt_number=3)
...
class TestCreateRetryConfiguration(unittest.TestCase):
    def setUp(self):
        self.retry_config = {
            ...
            "OperationFoo": {
                "policies": {
                    "crc32check": {
                        "applies_when": {"response": {"crc32body": "x-amz-crc32",}}
                    }
                }
            },
            "OperationBar": {
    ...
    def test_create_retry_single_checker_service_level(self):
        ...
        self.assertIsInstance(checker._checker, retryhandler.ServiceErrorCodeChecker)
    ...
    def test_retry_with_socket_errors(self):
        ...
        self.assertIsInstance(all_checkers[0], retryhandler.ServiceErrorCodeChecker)
    ...
    def test_create_retry_handler_with_socket_errors(self):
        ...
        sleep_time = handler(response=None, attempts=1, caught_exception=exception)
    ...
    def test_crc32_check_propogates_error(self):
        ...
        self.assertEqual(
            handler(response=(http_response, {}), attempts=1, caught_exception=None), 1
        )
        with self.assertRaises(ChecksumError):
            handler(response=(http_response, {}), attempts=10, caught_exception=None)
...
class TestRetryHandler(unittest.TestCase):
    ...
    def test_none_response_when_no_matches(self):
        ...
        self.assertIsNone(handler(response=response, attempts=1, caught_exception=None))
...
class TestCRC32Checker(unittest.TestCase):
    ...
    def test_crc32_matches(self):
        ...
        self.assertIsNone(
            self.checker(
                response=(http_response, {}), attempt_number=1, caught_exception=None
            )
        )
...
class TestDelayExponential(unittest.TestCase):
    def test_delay_with_numeric_base(self):
        self.assertEqual(
            retryhandler.delay_exponential(base=3, growth_factor=2, attempts=3), 12
        )

    def test_delay_with_rand_string(self):
        delay = retryhandler.delay_exponential(base="rand", growth_factor=2, attempts=3)
        # 2 ** (3 - 1) == 4, so the retry is between 0, 4.
        self.assertTrue(0 <= delay <= 4)

    def test_value_error_raised_with_non_positive_number(self):
        with self.assertRaises(ValueError):
            retryhandler.delay_exponential(base=-1, growth_factor=2, attempts=3)
