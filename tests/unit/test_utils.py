...
class TestResolveIMDSEndpointMode(unittest.TestCase):
    def create_session_with_config(self, endpoint_mode, imds_use_IPv6):
        ...
        session.set_config_variable("ec2_metadata_service_endpoint_mode", endpoint_mode)
...
class TestTransformName(unittest.TestCase):
    ...
    def test_special_cases(self):
        ...
        self.assertEqual(
            xform_name("CreateCachediSCSIVolume", "-"), "create-cached-iscsi-volume"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            xform_name("CreateStorediSCSIVolume", "-"), "create-stored-iscsi-volume"
        )
        self.assertEqual(xform_name("sourceServerIDs", "-"), "source-server-ids")
...
class TestValidateJMESPathForSet(unittest.TestCase):
    def setUp(self):
        ...
        self.data = {
            "Response": {"Thing": {"Id": 1, "Name": "Thing #1",}},
            "Marker": "some-token",
        }
...
class TestParseEC2CredentialsFile(unittest.TestCase):
    ...
    def test_multiple_equals_on_line(self):
        ...
        self.assertEqual(
            parse_key_val_file_contents(contents),
            {"AWSAccessKeyId": "a", "AWSSecretKey": "secret_key_with_equals=b"},
        )
...
class TestParseTimestamps(unittest.TestCase):
    ...
    def test_parse_epoch_zero_time(self):
        self.assertEqual(
            parse_timestamp(0), datetime.datetime(1970, 1, 1, 0, 0, 0, tzinfo=tzutc())
        )
...
class TestDatetime2Timestamp(unittest.TestCase):
    def test_datetime2timestamp_naive(self):
        self.assertEqual(datetime2timestamp(datetime.datetime(1970, 1, 2)), 86400)

    def test_datetime2timestamp_aware(self):
        tzinfo = tzoffset("BRST", -10800)
        self.assertEqual(
            datetime2timestamp(datetime.datetime(1970, 1, 2, tzinfo=tzinfo)), 97200
        )
...
class TestParseToUTCDatetime(unittest.TestCase):
    ...
    def test_handles_full_iso_8601(self):
        expected = datetime.datetime(1970, 1, 1, 0, 0, 0, tzinfo=tzutc())
        self.assertEqual(parse_to_aware_datetime("1970-01-01T00:00:00Z"), expected)
...
class TestArgumentGenerator(unittest.TestCase):
    ...
    def test_will_use_member_names_for_string_values(self):
        ...
        self.assert_skeleton_from_model_is(
            ...
            generated_skeleton={"A": "A", "B": 0, "C": 0.0, "D": True,},
        )
        ...
        shape_map = ShapeResolver(
            {
                "InputShape": {
                    "type": "structure",
                    "members": {"StringList": {"shape": "StringList"},},
                },
                "StringList": {"type": "list", "member": {"shape": "StringType"},},
    ...
    def test_generate_scalar_list(self):
        self.assert_skeleton_from_model_is(
            model={"A": {"type": "list", "member": {"type": "string"}},},
...
class TestIsValidEndpointURL(unittest.TestCase):
    ...
    def test_path_component_ignored(self):
        self.assertTrue(is_valid_endpoint_url("https://foo.bar.com/other/path/"))
...
class TestFixS3Host(unittest.TestCase):
    ...
    def test_dns_style_not_used_for_get_bucket_location(self):
        ...
        request = AWSRequest(method="GET", headers={}, url=original_url,)
        signature_version = "s3"
        region_name = "us-west-2"
        fix_s3_host(
            request=request,
    ...
    def test_can_provide_default_endpoint_url(self):
        ...
        self.assertEqual(request.url, "https://bucket.foo.s3.amazonaws.com/key.txt")
...
class TestSwitchToVirtualHostStyle(unittest.TestCase):
    def test_switch_to_virtual_host_style(self):
        request = AWSRequest(
            method="PUT", headers={}, url="https://foo.amazonaws.com/bucket/key.txt"
        )
        region_name = "us-west-2"
        signature_version = "s3"
        ...
        self.assertEqual(request.url, "https://bucket.foo.amazonaws.com/key.txt")
        self.assertEqual(request.auth_path, "/bucket/key.txt")
...
class TestSwitchToChunkedEncodingForNonSeekableObjects(unittest.TestCase):
    def test_switch_to_chunked_encodeing_for_stream_like_object(self):
        ...
        self.assertEqual(prepared_request.headers, {"Transfer-Encoding": "chunked"})
...
class TestGetServiceModuleName(unittest.TestCase):
    def setUp(self):
        ...
        self.service_model = ServiceModel(self.service_description, "myservice")

    def test_default(self):
        self.assertEqual(get_service_module_name(self.service_model), "MyService")

    def test_client_name_with_amazon(self):
        self.service_description["metadata"]["serviceFullName"] = "Amazon MyService"
        self.assertEqual(get_service_module_name(self.service_model), "MyService")

    def test_client_name_using_abreviation(self):
        self.service_description["metadata"]["serviceAbbreviation"] = "Abbreviation"
        self.assertEqual(get_service_module_name(self.service_model), "Abbreviation")

    def test_client_name_with_non_alphabet_characters(self):
        self.service_description["metadata"]["serviceFullName"] = "Amazon My-Service"
        self.assertEqual(get_service_module_name(self.service_model), "MyService")
...
class TestSwitchHostS3Accelerate(unittest.TestCase):
    def setUp(self):
        ...
        self.request = AWSRequest(method="PUT", headers={}, url=self.original_url)
...
class TestDeepMerge(unittest.TestCase):
    ...
    def test_deep_merge(self):
        ...
        b = {
            "first": {
                "second": {"otherkey": "newvalue", "yetanotherkey": "yetanothervalue"}
            }
        }
        deep_merge(a, b)
...
class TestS3RegionRedirector(unittest.TestCase):
    ...
    def set_client_response_headers(self, headers):
        ...
        self.client.head_bucket.side_effect = [error_response, success_response]

    def test_set_request_url(self):
        params = {"url": "https://us-west-2.amazonaws.com/foo"}
        context = {"signing": {"endpoint": "https://eu-central-1.amazonaws.com"}}
        self.redirector.set_request_url(params, context)
        self.assertEqual(params["url"], "https://eu-central-1.amazonaws.com/foo")
    ...
    def test_set_request_url_keeps_old_scheme(self):
        ...
        context = {"signing": {"endpoint": "https://eu-central-1.amazonaws.com"}}
        self.redirector.set_request_url(params, context)
        self.assertEqual(params["url"], "http://eu-central-1.amazonaws.com/foo")
    ...
    def test_redirect_from_error(self):
        ...
        self.assertEqual(request_dict["url"], "https://eu-central-1.amazonaws.com/foo")
    ...
    def test_get_region_from_head_bucket_error(self):
        self.set_client_response_headers({"x-amz-bucket-region": "eu-central-1"})
        response = (
            None,
            {
                "Error": {
...
class TestS3ArnParamHandler(unittest.TestCase):
    ...
    def test_accesspoint_arn(self):
        params = {"Bucket": "arn:aws:s3:us-west-2:123456789012:accesspoint/endpoint"}
        context = {}
    ...
    def test_errors_for_non_accesspoint_arn(self):
        params = {"Bucket": "arn:aws:s3:us-west-2:123456789012:unsupported:resource"}
        context = {}
...
class TestS3EndpointSetter(unittest.TestCase):
    ...
    def get_s3_outpost_request(self, **s3_request_kwargs):
        request = self.get_s3_request(self.accesspoint_name, **s3_request_kwargs)
    ...
    def get_s3_accesspoint_request(
        self, accesspoint_name=None, accesspoint_context=None, **s3_request_kwargs
    ):
        ...
        if accesspoint_context is None:
            accesspoint_context = self.get_s3_accesspoint_context(name=accesspoint_name)
        request.context["s3_accesspoint"] = accesspoint_context
        return request
    ...
    def test_accesspoint_supports_custom_endpoint(self):
        endpoint_setter = self.get_endpoint_setter(endpoint_url="https://custom.com")
        request = self.get_s3_accesspoint_request()
...
class TestInstanceMetadataFetcher(unittest.TestCase):
    ...
    def add_get_token_imds_response(self, token, status_code=200):
        self.add_imds_response(body=token.encode("utf-8"), status_code=status_code)
    ...
    def test_ec2_metadata_endpoint_service_mode(self):
        configs = [
            ({"ec2_metadata_service_endpoint_mode": "ipv6"}, "http://[fd00:ec2::254]/"),
            ({"ec2_metadata_service_endpoint_mode": "ipv6"}, "http://[fd00:ec2::254]/"),
            ({"ec2_metadata_service_endpoint_mode": "ipv4"}, "http://169.254.169.254/"),
            ({"ec2_metadata_service_endpoint_mode": "foo"}, "http://169.254.169.254/"),
            (
                {
    ...
    def test_includes_user_agent_header(self):
        ...
        InstanceMetadataFetcher(user_agent=user_agent).retrieve_iam_role_credentials()
    ...
    def test_non_200_response_for_role_name_is_retried(self):
        ...
        # be retried.
        self.add_get_token_imds_response(token="token")
        self.add_imds_response(status_code=429, body=b'{"message": "Slow down"}')
        self.add_get_role_name_imds_response()
        self.add_get_credentials_imds_response()
        result = InstanceMetadataFetcher(num_attempts=2).retrieve_iam_role_credentials()
        self.assertEqual(result, self._expected_creds)
    ...
    def test_exhaust_retries_on_role_name_request(self):
        self.add_get_token_imds_response(token="token")
        self.add_imds_response(status_code=400, body=b"")
        result = InstanceMetadataFetcher(num_attempts=1).retrieve_iam_role_credentials()
        self.assertEqual(result, {})
