...
class TestMonitor(unittest.TestCase):
    ...
    def test_handle_catches_exceptions(self):
        ...
        try:
            ...
            self.fail(
                "The publishing exception should have been caught " "in the handler"
            )
...
class TestMonitorEventAdapter(unittest.TestCase):
    ...
    def feed_request_created_event(self, current_time=0):
        self.mock_time.return_value = current_time
        self.adapter.feed("request-created", {"request": self.request,})
    ...
    def test_feed_with_wire_exception(self):
        ...
        self.mock_time.return_value = 4
        call_event = self.adapter.feed(
            "after-call-error", {"exception": wire_exception, "context": self.context}
        )
        self.assertEqual(
            call_event,
            APICallEvent(
...
class TestBaseMonitorEvent(unittest.TestCase):
    ...
    def test_not_eq_different_attrs(self):
        self.assertNotEqual(
            ...
            BaseMonitorEvent(
                service="DifferentService", operation="DifferentOperation", timestamp=0
            ),
        )
...
class TestCSMSerializer(unittest.TestCase):
    ...
    def test_serialize_api_call_event_with_latency(self):
        event = APICallEvent(
            service=self.service, operation=self.operation, timestamp=1000, latency=2000
        )
        serialized_event_dict = self.get_serialized_event_dict(event)
    ...
    def test_serialize_api_call_event_parsed_error(self):
        ...
        attempt = event.new_api_call_attempt(2000)
        attempt.parsed_error = {"Code": "MyErrorCode", "Message": "My error message"}
        serialized_event_dict = self.get_serialized_event_dict(event)
        self.assertEqual(serialized_event_dict["FinalAwsException"], "MyErrorCode")
        self.assertEqual(
            serialized_event_dict["FinalAwsExceptionMessage"], "My error message"
        )
    ...
    def test_serialize_api_call_event_wire_exception(self):
        ...
        serialized_event_dict = self.get_serialized_event_dict(event)
        self.assertEqual(serialized_event_dict["FinalSdkException"], "Exception")
        self.assertEqual(
            serialized_event_dict["FinalSdkExceptionMessage"], "Error on the wire"
        )
    ...
    def test_serialize_api_call_attempt_event(self):
        event = APICallAttemptEvent(
            service=self.service, operation=self.operation, timestamp=self.timestamp
        )
        serialized_event_dict = self.get_serialized_event_dict(event)
        self.assertEqual(
            serialized_event_dict,
    ...
    def test_serialize_with_session_token(self):
        ...
        serialized_event_dict = self.get_serialized_event_dict(event)
        self.assertEqual(serialized_event_dict["SessionToken"], "my-security-token")
    ...
    def test_serialize_with_service_error(self):
        event = APICallAttemptEvent(
            ...
            operation=self.operation,
            timestamp=self.timestamp,
            parsed_error={"Code": "MyErrorCode", "Message": "My error message"},
        )
        serialized_event_dict = self.get_serialized_event_dict(event)
    ...
    def test_serialize_truncates_long_user_agent(self):
        ...
        serialized_event_dict = self.get_serialized_event_dict(event)
        self.assertEqual(
            serialized_event_dict["UserAgent"], user_agent[:max_user_agent_length]
        )
    ...
    def test_serialize_truncates_long_service_error(self):
        ...
        event = APICallAttemptEvent(
            ...
            operation=self.operation,
            timestamp=self.timestamp,
            parsed_error={"Code": long_error_code, "Message": long_error_message},
        )
        serialized_event_dict = self.get_serialized_event_dict(event)
        self.assertEqual(
