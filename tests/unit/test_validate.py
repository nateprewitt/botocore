...
class BaseTestValidate(unittest.TestCase):
    def assert_has_validation_errors(self, given_shapes, input_params, errors):
        ...
        errors_found = self.get_validation_error_message(given_shapes, input_params)
...
class TestValidateJSONValueTrait(BaseTestValidate):
    def test_accepts_jsonvalue_string(self):
        ...
        errors = self.get_validation_error_message(
            ...
            input_params={"json": {"data": [1, 2.3, "3"], "unicode": u"\u2713"}},
        )
...
class TestValidateDocumentType(BaseTestValidate):
    def test_accepts_document_type_string(self):
        self.shapes = {
            "Input": {
                "type": "structure",
                "members": {"inlineDocument": {"shape": "DocumentType",}},
            },
...
class TestValidateTypes(BaseTestValidate):
    ...
    def test_datetime_type_accepts_datetime_obj(self):
        errors = self.get_validation_error_message(
            given_shapes=self.shapes, input_params={"Timestamp": datetime.now(),}
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
    ...
    def test_datetime_accepts_string_timestamp(self):
        errors = self.get_validation_error_message(
            given_shapes=self.shapes, input_params={"Timestamp": "2014-01-01 12:00:00"}
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
...
class TestValidateRanges(BaseTestValidate):
    def setUp(self):
        self.shapes = {
            ...
            "IntType": {"type": "integer", "min": 0, "max": 1000,},
            "LongType": {"type": "long", "min": 0, "max": 1000,},
            "StringType": {"type": "string", "min": 1, "max": 10,},
    ...
    def test_does_not_validate_greater_than_range(self):
        errors = self.get_validation_error_message(
            given_shapes=self.shapes,
            input_params={"Int": 100000000, "Long": 100000000,},
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
    ...
    def test_string_min_length_contraint(self):
        self.assert_has_validation_errors(
            given_shapes=self.shapes,
            input_params={"String": "",},
            errors=["Invalid length for parameter String",],
        )

    def test_does_not_validate_string_max_length_contraint(self):
        errors = self.get_validation_error_message(
            given_shapes=self.shapes,
            input_params={"String": "more than ten characters",},
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
    ...
    def test_list_min_length_constraint(self):
        self.assert_has_validation_errors(
            given_shapes=self.shapes,
            input_params={"List": [],},
            errors=["Invalid length for parameter List",],
        )

    def test_does_not_validate_list_max_length_constraint(self):
        errors = self.get_validation_error_message(
            given_shapes=self.shapes,
            input_params={"List": ["a", "b", "c", "d", "e", "f", "g", "h"],},
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
    ...
    def test_only_min_value_specified(self):
        ...
        self.assert_has_validation_errors(
            given_shapes=self.shapes,
            input_params={"OnlyMin": "",},
            errors=["Invalid length for parameter OnlyMin",],
        )

    def test_does_not_validate_max_when_only_max_value_specified(self):
        errors = self.get_validation_error_message(
            given_shapes=self.shapes,
            input_params={"OnlyMax": "more than ten characters",},
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
...
class TestValidateMapType(BaseTestValidate):
    def setUp(self):
        self.shapes = {
            "Input": {"type": "structure", "members": {"Map": {"shape": "MapType"},}},
            "MapType": {
                "type": "map",
                "key": {"shape": "StringType"},
                "value": {"shape": "StringType"},
            },
            "StringType": {"type": "string", "min": 2,},
        }
    ...
    def test_validate_keys_and_values(self):
        self.assert_has_validation_errors(
            ...
            errors=["Invalid length for parameter Map",],
        )
...
class TestValidationFloatType(BaseTestValidate):
    def setUp(self):
        self.shapes = {
            "Input": {
                "type": "structure",
                "members": {"Float": {"shape": "FloatType"},},
            },
            "FloatType": {"type": "float", "min": 2, "max": 5,},
        }

    def test_range_float(self):
        self.assert_has_validation_errors(
            given_shapes=self.shapes,
            input_params={"Float": 1,},
            errors=["Invalid value for parameter Float",],
        )

    def test_decimal_allowed(self):
        errors = self.get_validation_error_message(
            given_shapes=self.shapes, input_params={"Float": decimal.Decimal("2.12345")}
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
    ...
    def test_decimal_still_validates_range(self):
        self.assert_has_validation_errors(
            given_shapes=self.shapes,
            input_params={"Float": decimal.Decimal("1"),},
            errors=["Invalid value for parameter Float",],
        )
...
class TestValidateTypeBlob(BaseTestValidate):
    def setUp(self):
        self.shapes = {
            "Input": {"type": "structure", "members": {"Blob": {"shape": "BlobType"},}},
            "BlobType": {"type": "blob", "min": 2, "max": 5,},
        }
    ...
    def test_validates_bytearray(self):
        errors = self.get_validation_error_message(
            given_shapes=self.shapes, input_params={"Blob": bytearray(b"12345")},
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
    ...
    def test_validates_file_like_object(self):
        ...
        errors = self.get_validation_error_message(
            given_shapes=self.shapes, input_params={"Blob": value},
        )
        error_msg = errors.generate_report()
        self.assertEqual(error_msg, "")
    ...
    def test_validate_type(self):
        self.assert_has_validation_errors(
            given_shapes=self.shapes,
            input_params={"Blob": 24,},
            errors=["Invalid type for parameter Blob",],
        )
