...
class TestAWSHTTPConnection(unittest.TestCase):
    ...
    def test_tunnel_readline_none_bugfix(self):
        ...
        conn = self.create_tunneled_connection(
            url="s3.amazonaws.com", port=443, response=b"HTTP/1.1 200 OK\r\n",
        )
        conn._tunnel()
    ...
    def test_encodes_unicode_method_line(self):
        ...
        conn.request(
            u"GET", "/bucket/foo", b"body", headers={"Utf8-Header": b"\xe5\xb0\x8f"}
        )
        response = conn.getresponse()
        self.assertEqual(response.status, 200)
...
class TestPrepareRequestDict(unittest.TestCase):
    ...
    def test_prepare_request_dict_for_get(self):
        request_dict = {"method": u"GET", "url_path": "/"}
        ...
        self.assertEqual(self.base_request_dict["url"], "https://s3.amazonaws.com/")
        self.assertEqual(
    ...
    def test_url_path_combined_with_endpoint_url(self):
        ...
        request_dict = {"query_string": {u"prefix": u"foo"}, "url_path": u"/mybucket"}
    ...
    def test_url_path_with_trailing_slash(self):
        self.prepare_base_request_dict(
            {"url_path": u"/mybucket"}, endpoint_url="https://custom.endpoint/foo/bar/"
        )

        self.assertEqual(
            self.base_request_dict["url"], "https://custom.endpoint/foo/bar/mybucket"
        )

    def test_url_path_is_slash(self):
        self.prepare_base_request_dict(
            {"url_path": u"/"}, endpoint_url="https://custom.endpoint/foo/bar/"
        )
