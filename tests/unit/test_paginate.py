...
class TestPagination(unittest.TestCase):
    ...
    def test_result_key_available(self):
        self.assertEqual([rk.expression for rk in self.paginator.result_keys], ["Foo"])
    ...
    def test_next_token_in_response(self):
        ...
        self.assertEqual(
            ...
            [mock.call(), mock.call(NextToken="token1"), mock.call(NextToken="token2")],
        )
    ...
    def test_build_multiple_results(self):
        ...
        expected_token = encode_token({"Marker": "m2", "boto_truncate_amount": 2,})
...
class TestPaginatorPageSize(unittest.TestCase):
    ...
    def test_page_size(self):
        kwargs = {"arg1": "foo", "arg2": "bar", "PaginationConfig": {"PageSize": 5}}
...
class TestPaginatorWithPathExpressions(unittest.TestCase):
    def setUp(self):
        ...
        self.paginate_config = {
            "output_token": ["NextMarker || ListBucketResult.Contents[-1].Key"],
    ...
    def test_s3_list_object_complex(self):
        responses = [
            {"NextMarker": "token1"},
            {"ListBucketResult": {"Contents": [{"Key": "first"}, {"Key": "Last"}]}},
            {"not_next_token": "foo"},
        ]
        self.method.side_effect = responses
        list(self.paginator.paginate())
...
class TestBinaryTokens(unittest.TestCase):
    ...
    def test_build_full_result_with_bytes(self):
        ...
        complete = pages.build_full_result()
        expected_token = encode_token({"Marker": b"\xff", "boto_truncate_amount": 1,})
        expected_response = {
    ...
    def test_build_full_result_with_multiple_bytes_values(self):
        responses = [
            {"Users": ["User1", "User2"], "Marker": {"key": b"\xff", "key2": b"\xef"}},
            {"Users": ["User3", "User4"], "Marker": {"key": b"\xfe", "key2": b"\xee"}},
            {"Users": ["User5"]},
        ]
        self.method.side_effect = responses
        pages = self.paginator.paginate(PaginationConfig={"MaxItems": 3})
        complete = pages.build_full_result()
        expected_token = encode_token(
            {"Marker": {"key": b"\xff", "key2": b"\xef"}, "boto_truncate_amount": 1,}
        )
        expected_response = {
            "Users": ["User1", "User2", "User3"],
            "NextToken": expected_token,
    ...
    def test_resume_with_bytes(self):
        ...
        self.method.side_effect = responses
        starting_token = encode_token({"Marker": b"\xff", "boto_truncate_amount": 1,})
        pages = self.paginator.paginate(
...
class TestKeyIterators(unittest.TestCase):
    ...
    def test_max_items_can_be_specified(self):
        ...
        self.assertEqual(
            paginator.paginate(PaginationConfig={"MaxItems": 1}).build_full_result(),
    ...
    def test_next_token_on_page_boundary(self):
        ...
        self.assertEqual(
            paginator.paginate(PaginationConfig={"MaxItems": 2}).build_full_result(),
    ...
    def test_max_items_can_be_specified_truncates_response(self):
        ...
        self.method.side_effect = responses
        expected_token = encode_token({"Marker": "m1", "boto_truncate_amount": 1})
        self.assertEqual(
            paginator.paginate(PaginationConfig={"MaxItems": 4}).build_full_result(),
            {
    ...
    def test_resume_next_marker_mid_page(self):
        ...
        self.method.side_effect = responses
        starting_token = encode_token({"Marker": "m1", "boto_truncate_amount": 1})
        pagination_config = {"StartingToken": starting_token}
        self.assertEqual(
            paginator.paginate(PaginationConfig=pagination_config).build_full_result(),
    ...
    def test_max_items_exceeds_actual_amount(self):
        ...
        self.method.side_effect = responses
        self.assertEqual(
            paginator.paginate(PaginationConfig={"MaxItems": 10}).build_full_result(),
            {"Users": ["User1", "User2", "User3"]},
        )
...
class TestMultipleResultKeys(unittest.TestCase):
    ...
    def test_build_full_result_with_different_length_result_keys(self):
        ...
        complete = pages.build_full_result()
        self.assertEqual(
            complete, {"Users": ["User1"], "Groups": ["Group1", "Group2", "Group3"]}
        )
    ...
    def test_build_result_with_secondary_keys(self):
        ...
        complete = pages.build_full_result()
        expected_token = encode_token({"Marker": None, "boto_truncate_amount": 1})
        self.assertEqual(
            complete,
            {
                "Users": ["User1"],
    ...
    def test_resume_with_secondary_keys(self):
        ...
        self.method.side_effect = responses
        starting_token = encode_token({"Marker": None, "boto_truncate_amount": 1})
        pages = self.paginator.paginate(
        ...
        expected_token = encode_token({"Marker": "m1"})
        self.assertEqual(
            complete, {"Users": ["User2"], "Groups": [], "NextToken": expected_token}
        )
...
class TestMultipleInputKeys(unittest.TestCase):
    ...
    def test_resume_with_multiple_input_keys(self):
        ...
        self.assertEqual(
            complete, {"Users": ["User4"], "Groups": [], "NextToken": expected_token}
        )
        self.assertEqual(
            self.method.call_args_list, [mock.call(InMarker1="m1", InMarker2="m2")]
        )
    ...
    def test_result_key_exposed_on_paginator(self):
        self.assertEqual(
            [rk.expression for rk in self.paginator.result_keys], ["Users", "Groups"]
        )
...
class TestExpressionKeyIterators(unittest.TestCase):
    def setUp(self):
        ...
        self.responses = [
            ...
            {"EngineDefaults": {"Parameters": ["Three", "Four"]}, "Marker": "m2"},
    ...
    def test_result_key_iters(self):
        ...
        iterators = pages.result_key_iters()
        self.assertEqual(len(iterators), 1)
        self.assertEqual(list(iterators[0]), ["One", "Two", "Three", "Four", "Five"])
...
class TestIncludeNonResultKeys(unittest.TestCase):
    ...
    def test_include_with_multiple_result_keys(self):
        ...
        self.method.side_effect = [
            ...
            {"ResultKey1": ["e", "f"], "ResultKey2": ["y", "z"], "NotResultKey": "a"},
        ]
        pages = self.paginator.paginate()
        actual = pages.build_full_result()
        expected = {
    ...
    def test_include_with_nested_result_keys(self):
        ...
        actual = pages.build_full_result()
        self.assertEqual(
            pages.non_aggregate_part, {"Outer": "v2", "Result": {"Inner": "v1"}}
        )
        expected = {
...
class TestSearchOverResults(unittest.TestCase):
    def setUp(self):
        ...
        self.paginator = Paginator(self.method, self.paginate_config, self.model)
        responses = [
            {"Foo": [{"a": 1}, {"b": 2}], "IsTruncated": True, "NextToken": "1"},
            {"Foo": [{"a": 3}, {"b": 4}], "IsTruncated": True, "NextToken": "2"},
...
class TestDeprecatedStartingToken(unittest.TestCase):
    ...
    def test_deprecated_multiple_starting_token(self):
        ...
        self.assert_pagination_result(expected, pagination_config, multiple_tokens=True)
