...
class TestAutoGeneratedClient(unittest.TestCase):
    def setUp(self):
        ...
        self.endpoint_creator_patch = mock.patch("botocore.args.EndpointCreator")
    ...
    def create_client_creator(
        ...
        if endpoint_prefix is not None:
            self.service_description["metadata"]["endpointPrefix"] = endpoint_prefix
    ...
    def assert_no_param_error_raised(self, client):
        try:
            ...
            self.fail(
                "ParamValidationError shouldn't be raised " "with validation disabled"
            )
    ...
    def test_client_has_modeled_exceptions(self):
        ...
        self.assertTrue(
            issubclass(
                service_client.exceptions.TestOperationException, client.ClientError
            )
        )
    ...
    def test_create_client_class_forwards_api_version(self):
        creator = self.create_client_creator()
        specific_api_version = "2014-03-01"
        creator.create_client_class("myservice", api_version=specific_api_version)
        self.loader.load_service_model.assert_called_with(
            "myservice", "service-2", api_version=specific_api_version
        )
    ...
    def test_client_signature_no_override(self, request_signer):
        creator = self.create_client_creator()
        creator.create_client(
            "myservice", "us-west-2", credentials=self.credentials, scoped_config={}
        )
    ...
    def test_client_signature_override_config_file(self, request_signer):
        ...
        creator.create_client(
            "myservice", "us-west-2", credentials=self.credentials, scoped_config=config
        )
        request_signer.assert_called_with(
    ...
    def test_client_signature_override_arg(self, request_signer):
        ...
        creator.create_client(
            "myservice", "us-west-2", credentials=self.credentials, client_config=config
        )
        request_signer.assert_called_with(
            mock.ANY, mock.ANY, mock.ANY, "foo", mock.ANY, mock.ANY
        )
    ...
    def test_client_registers_request_created_handler(self):
        ...
        creator.create_client("myservice", "us-west-2", credentials=self.credentials)
        self.assertIn(
    ...
    def test_client_with_custom_neither_timeout(self):
        ...
        call_kwargs = self.endpoint_creator.create_endpoint.call_args[1]
        self.assertEqual(call_kwargs["timeout"], (DEFAULT_TIMEOUT, DEFAULT_TIMEOUT))

    def test_client_with_custom_params(self):
        creator = self.create_client_creator()
        creator.create_client("myservice", "us-west-2", is_secure=False, verify=False)
        call_kwargs = self.endpoint_creator.create_endpoint.call_args[1]
    ...
    def test_waiter_config_uses_service_name_not_endpoint_prefix(self):
        ...
        service_client = creator.create_client("other-service-name", "us-west-2")
    ...
    def test_service_has_retry_event(self):
        ...
        event_emitter.register.assert_any_call(
            "needs-retry.myservice", mock.ANY, unique_id="retry-config-myservice"
        )
    ...
    def test_service_registers_retry_handler(self):
        ...
        creator = self.create_client_creator(
            event_emitter=event_emitter, retry_handler_factory=retry_handler_factory
        )
        creator.create_client("myservice", "us-west-2")

        event_emitter.register.assert_any_call(mock.ANY, handler, unique_id=mock.ANY)
    ...
    def test_emits_after_call_error(self):
        ...
        event_emitter.register("after-call-error.myservice.TestOperation", record)
    ...
    def test_can_register_standard_retry_mode(self):
        with mock.patch("botocore.client.standard") as standard:
            ...
            creator.create_client(
                "myservice",
                "us-west-2",
                client_config=botocore.config.Config(retries={"mode": "standard"}),
            )
    ...
    def test_client_s3_addressing_style_with_bad_value(self):
        creator = self.create_client_creator()
        client = creator.create_client(
            "myservice", "us-west-2", scoped_config={"s3": ""},
        )
        self.assertIsNone(client.meta.config.s3)

    def test_client_s3_addressing_style_with_config_store(self):
        self.config_store.set_config_variable("s3", {"addressing_style": "virtual"})
        creator = self.create_client_creator()
        client = creator.create_client("myservice", "us-west-2")
    ...
    def test_client_payload_signing_from_config_store(self):
        self.config_store.set_config_variable("s3", {"payload_signing_enabled": True})
        creator = self.create_client_creator()
        my_client = creator.create_client("myservice", "us-west-2")
        self.assertEqual(my_client.meta.config.s3["payload_signing_enabled"], True)
    ...
    def test_client_s3_accelerate_from_config_store(self):
        ...
        creator = self.create_client_creator()
        my_client = creator.create_client("myservice", "us-west-2")
        self.assertEqual(my_client.meta.config.s3["use_accelerate_endpoint"], True)
    ...
    def test_getattr_emits_event(self):
        ...
        emitter.emit_until_response.assert_called_once_with(
            "getattr.myservice.attribute_that_does_not_exist", client=service_client
        )
    ...
    def test_exception_classes_across_clients_are_the_same(self):
        ...
        with Stubber(client) as stub:
            ...
            try:
                ...
                self.assertIsInstance(e, client.exceptions.TestOperationException)
...
class TestClientEndpointBridge(unittest.TestCase):
    ...
    def test_can_guess_endpoint_and_use_given_endpoint_url(self):
        ...
        resolver.construct_endpoint.return_value = None
        bridge = ClientEndpointBridge(resolver)
        resolved = bridge.resolve("test", "guess", endpoint_url="http://test.com")
        self.assertEqual("guess", resolved["region_name"])
        self.assertEqual("guess", resolved["signing_region"])
    ...
    def test_can_use_endpoint_url_with_resolved_endpoint(self):
        ...
        bridge = ClientEndpointBridge(resolver)
        resolved = bridge.resolve("ec2", "us-west-2", endpoint_url="https://foo")
    ...
    def test_can_construct_dualstack_endpoint_when_enabled(self):
        ...
        self.assertEqual(
            resolved["endpoint_url"], "https://s3.dualstack.us-east-1.amazonaws.com"
        )
    ...
    def test_dualstack_honors_dns_suffix(self):
        ...
        self.assertEqual(
            resolved["endpoint_url"], "https://s3.dualstack.cn-north-1.amazonaws.com.cn"
        )
