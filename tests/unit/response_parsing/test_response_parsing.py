    ...
    for dp in ["responses", "errors"]:
        ...
        for service_name in service_names:
            ...
            for xmlfile in service_xml_files:
                ...
                yield _test_parsed_response, xmlfile, raw_response_body, operation_model, expected
...
def test_json_errors_parsing():
    ...
    for json_response_file in os.listdir(json_responses_dir):
        ...
        service_name, operation_name = os.path.splitext(json_response_file)[0].split(
            "-", 1
        )
        expected_parsed_response = os.path.join(expected_parsed_dir, json_response_file)
        raw_response_file = os.path.join(json_responses_dir, json_response_file)
        ...
        yield _test_parsed_response, raw_response_file, raw_response_body, operation_model, expected
...
def _uhg_test_json_parsing():
    ...
    for service_name in service_names:
        service_model = session.get_service_model(service_name)
        service_json_files = glob.glob("%s/%s-*.json" % (input_path, service_name))
        for jsonfile in service_json_files:
            ...
            with open(jsonfile, "rb") as f:
                raw_response_body = f.read()
            yield _test_parsed_response, jsonfile, raw_response_body, operation_model, expected
