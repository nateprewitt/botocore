...
class TestBinaryTypes(BaseModelWithBlob):
    ...
    def test_blob_accepts_str_type(self):
        body = u"ascii text"
        request = self.serialize_to_request(input_params={"Blob": body})
        self.assert_serialized_blob_equals(request, blob_bytes=body.encode("ascii"))

    def test_blob_handles_unicode_chars(self):
        body = u"\u2713"
        request = self.serialize_to_request(input_params={"Blob": body})
        self.assert_serialized_blob_equals(request, blob_bytes=body.encode("utf-8"))
...
class TestBinaryTypesJSON(BaseModelWithBlob):
    ...
    def test_blob_accepts_bytes_type(self):
        ...
        self.assertEqual(base64.b64encode(body).decode("ascii"), serialized_blob)
...
class TestTimestamps(unittest.TestCase):
    def setUp(self):
        self.model = {
            ...
            "shapes": {
                "InputShape": {
                    "type": "structure",
                    "members": {"Timestamp": {"shape": "TimestampType"},},
                },
                "TimestampType": {"type": "timestamp",},
            },
        }
    ...
    def test_microsecond_timestamp_without_tz_info(self):
        request = self.serialize_to_request({"Timestamp": "2014-01-01T12:12:12.123456"})
        self.assertEqual(request["body"]["Timestamp"], "2014-01-01T12:12:12.123456Z")
...
class TestJSONTimestampSerialization(unittest.TestCase):
    ...
    def test_accepts_partial_iso_format(self):
        body = json.loads(
            self.serialize_to_request({"Timestamp": "1970-01-01"})["body"].decode(
                "utf-8"
            )
        )
        self.assertEqual(body["Timestamp"], 0)
    ...
    def setUp(self):
        self.model = {
            ...
            "shapes": {
                "InputShape": {
                    "type": "structure",
                    "members": {"Timestamp": {"shape": "StringTestType"},},
                },
    ...
    def setUp(self):
        self.model = {
            ...
            "shapes": {
                "InputShape": {
                    "type": "structure",
                    "members": {"Foo": {"shape": "FooShape", "locationName": "Foo"},},
                    "payload": "Foo",
                },
                "FooShape": {"type": "list", "member": {"shape": "StringShape"}},
