    ...
    def _test_attribute_raise_exception(attr_name):
        try:
            ...
            raise AssertionError(
                ...
                "be raised, but %s was raised instead" % (e.__class__)
            )
        else:
            raise AssertionError(
                "Expected UndefinedModelAttributeError to "
...
class TestServiceId(unittest.TestCase):
    def test_hypenize_replaces_spaces(self):
        self.assertEqual(model.ServiceId("my service").hyphenize(), "my-service")
...
class TestServiceModel(unittest.TestCase):
    def setUp(self):
        ...
        self.error_shapes = {
            "ExceptionOne": {"exception": True, "type": "structure", "members": {},},
            "ExceptionTwo": {
                "exception": True,
                "type": "structure",
                "members": {},
    ...
    def test_service_name_can_be_overriden(self):
        service_model = model.ServiceModel(self.model, service_name="myservice")
    ...
    def test_hyphenize_service_id(self):
        self.assertEqual(self.service_model.service_id.hyphenize(), "myservice")

    def test_service_id_does_not_exist(self):
        service_model = {
            "metadata": {"protocol": "query", "endpointPrefix": "endpoint-prefix",},
            "documentation": "Documentation value",
            "operations": {},
            "shapes": {"StringShape": {"type": "string"}},
        }
        ...
        with self.assertRaisesRegexp(model.UndefinedModelAttributeError, service_name):
    ...
    def test_documentation_exposed_as_property(self):
        self.assertEqual(self.service_model.documentation, "Documentation value")

    def test_shape_names(self):
        self.assertEqual(self.service_model.shape_names, ["StringShape"])

    def test_repr_has_service_name(self):
        self.assertEqual(repr(self.service_model), "ServiceModel(endpoint-prefix)")
...
class TestOperationModelFromService(unittest.TestCase):
    ...
    def test_name_from_service_model_when_differs_from_name(self):
        self.model["operations"]["Foo"] = self.model["operations"]["OperationName"]
        service_model = model.ServiceModel(self.model)
    ...
    def test_error_shapes(self):
        ...
        self.assertEqual(operation.error_shapes[0].name, "NoSuchResourceException")
...
class TestShapeResolver(unittest.TestCase):
    ...
    def test_serialization_cache(self):
        shape_map = {
            "Foo": {
                "type": "structure",
                "members": {"Baz": {"shape": "StringType", "locationName": "other"},},
            },
            "StringType": {"type": "string"},
        }
        resolver = model.ShapeResolver(shape_map)
    ...
    def test_shape_overrides(self):
        shape_map = {
            "StringType": {"type": "string", "documentation": "Original documentation"}
        }
        resolver = model.ShapeResolver(shape_map)
    ...
    def test_shape_type_structure(self):
        ...
        self.assertEqual(list(sorted(shape.members)), ["NewPassword", "OldPassword"])
    ...
    def test_exception_error_code(self):
        shapes = {
            "FooException": {"exception": True, "type": "structure", "members": {}}
        }
    ...
    def test_shape_metadata(self):
        shapes = {
            ...
            "passwordType": {"type": "string", "min": 1, "max": 128, "sensitive": True},
        }
        resolver = model.ShapeResolver(shapes)
        shape = resolver.get_shape_by_name("ChangePasswordRequest")
        self.assertEqual(shape.metadata["required"], ["OldPassword", "NewPassword"])
    ...
    def test_error_shape_metadata(self):
        ...
        self.assertEqual(
            shape.metadata, {"exception": True, "retryable": {"throttling": True}}
        )
    ...
    def test_bad_shape_ref(self):
        ...
        shapes = {
            "Struct": {
                "type": "structure",
                "members": {"A": {"type": "string"}, "B": {"type": "string"},},
            }
        }
        resolver = model.ShapeResolver(shapes)
    ...
    def test_shape_name_in_repr(self):
        shapes = {"StringType": {"type": "string",}}
        resolver = model.ShapeResolver(shapes)
        self.assertIn("StringType", repr(resolver.get_shape_by_name("StringType")))
...
class TestBuilders(unittest.TestCase):
    ...
    def test_structure_shape_with_structure_type(self):
        b = model.DenormalizedStructureBuilder()
        shape = b.with_members(
            {"A": {"type": "structure", "members": {"A-1": {"type": "string"},}},}
        ).build_model()
        self.assertIsInstance(shape, model.StructureShape)
    ...
    def test_use_shape_name_when_provided(self):
        b = model.DenormalizedStructureBuilder()
        shape = b.with_members(
            {"A": {"type": "string", "shape_name": "MyStringShape",},}
        ).build_model()
    ...
    def test_unknown_shape_type(self):
        b = model.DenormalizedStructureBuilder()
        with self.assertRaises(model.InvalidShapeError):
            b.with_members({"A": {"type": "brand-new-shape-type",},}).build_model()
