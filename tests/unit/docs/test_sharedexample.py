...
class TestDocumentSharedExamples(BaseDocsTest):
    def setUp(self):
        ...
        self.add_shape(
            {"other": {"type": "structure", "members": {"nested": {"shape": "nested"}}}}
        )
        self.add_shape({"aloha": {"type": "list", "member": {"shape": "other"}}})
        ...
        self._examples = [
            {
                ...
                "input": OrderedDict(
                    [("aloha", ["other", {"nested": "fun!"}]), ("foo", "bar"),]
                ),
                "output": OrderedDict([("foo", "baz"),]),
                "comments": {
                    "input": {"aloha": "mahalo"},
...
class TestSharedExampleDocumenter(BaseDocsTest):
    ...
    def test_dict_example(self):
        self.add_shape(
            {"bar": {"type": "structure", "members": {"bar": {"shape": "String"}}}}
        )
    ...
    def test_list_example(self):
        self.add_shape({"foo": {"type": "list", "member": {"shape": "String"}}})
        self.add_shape_to_params("foo", "foo")
        self.documenter.document_shared_example(
            example={"input": {"foo": ["bar"]}},
            prefix="foo.bar",
    ...
    def test_timestamp_example(self):
        ...
        self.assert_contains_lines_in_order(
            ["foo.bar(", "    foo=datetime(2015, 11, 20, 21, 13, 12, 4, 324, 0)", ")"]
        )

    def test_map_example(self):
        self.add_shape({"baz": {"type": "string"}})
        self.add_shape(
            {"bar": {"type": "map", "key": {"shape": "baz"}, "value": {"shape": "baz"}}}
        )
        self.add_shape_to_params("foo", "bar")
        self.documenter.document_shared_example(
            example={"input": {"foo": {"bar": "baz"}}},
    ...
    def test_add_comment(self):
        self.add_shape_to_params("foo", "String")
        self.documenter.document_shared_example(
            example={"input": {"foo": "bar"}, "comments": {"input": {"foo": "baz"}}},
            prefix="foo.bar",
            section=self.doc_structure,
            operation_model=self.operation_model,
        )
    ...
    def test_unicode_exammple(self):
        ...
        self.assert_contains_lines_in_order([u"foo.bar(", u"    foo='\u2713'", u")"])
