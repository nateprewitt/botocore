...
class TestGetOfficialServiceName(BaseDocsTest):
    ...
    def test_no_short_name(self):
        self.assertEqual("Official Name", get_official_service_name(self.service_model))

    def test_aws_short_name(self):
        self.service_model.metadata["serviceAbbreviation"] = "AWS Foo"
        self.assertEqual(
            "Official Name (Foo)", get_official_service_name(self.service_model)
        )
...
class TestAutopopulatedParam(BaseDocsTest):
    ...
    def test_param_not_in_section_request_param(self):
        ...
        self.assertEqual("", self.doc_structure.flush_structure().decode("utf-8"))
...
class TestHideParamFromOperations(BaseDocsTest):
    def setUp(self):
        ...
        self.name = "MyMember"
        self.param = HideParamFromOperations("s3", self.name, ["SampleOperation"])
...
class TestAppendParamDocumentation(BaseDocsTest):
    ...
    def test_appends_documentation(self):
        ...
        self.param.append_documentation("docs.request-params", self.doc_structure)
