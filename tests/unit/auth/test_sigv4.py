...
class _SignatureTestCase(object):
    def __init__(self, test_case):
        filepath = os.path.join(TESTSUITE_DIR, test_case, os.path.basename(test_case))
        ...
        self.canonical_request = (
            io.open(filepath + ".creq", encoding="utf-8").read().replace("\r", "")
        )
        self.string_to_sign = (
            io.open(filepath + ".sts", encoding="utf-8").read().replace("\r", "")
        )
        self.authorization_header = (
            io.open(filepath + ".authz", encoding="utf-8").read().replace("\r", "")
        )
        self.signed_request = io.open(filepath + ".sreq", encoding="utf-8").read()
