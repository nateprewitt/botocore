...
SERVICE_DESCRIPTION_WITH_RETRIES = {
    ...
    "shapes": {
        "FakeInputOutputShape": {"type": "structure", "members": {},},
...
def _verify_retryable(checker, operation_model, status_code, error, is_retryable):
...
class TestRetryHandler(unittest.TestCase):
    ...
    def test_does_need_retry(self):
        ...
        self.retry_policy.should_retry.assert_called_with(mock.sentinel.retry_context)
...
class TestRetryEventAdapter(unittest.TestCase):
    ...
    def test_create_context_from_success_response(self):
        ...
        self.assertEqual(context.operation_model, mock.sentinel.operation_model)
    ...
    def test_can_inject_metadata_back_to_context(self):
        ...
        self.assertEqual(
            self.failed_response["ResponseMetadata"]["MaxAttemptsReached"], True
        )
...
class TestRetryPolicy(unittest.TestCase):
    ...
    def test_delegates_to_retry_checker(self):
        ...
        self.retry_checker.is_retryable.assert_called_with(mock.sentinel.context)
    ...
    def test_delegates_to_retry_backoff(self):
        ...
        self.retry_backoff.delay_amount.assert_called_with(mock.sentinel.context)


class TestExponentialBackoff(unittest.TestCase):
    def setUp(self):
        self.random = lambda: 1
        self.backoff = standard.ExponentialBackoff(max_backoff=20, random=self.random)
...
class TestRetryQuotaChecker(unittest.TestCase):
    ...
    def test_can_acquire_quota_non_timeout_error(self):
        self.assertTrue(self.quota_checker.acquire_retry_quota(self.create_context()))
    ...
    def test_acquire_quota_fails(self):
        quota_checker = standard.RetryQuotaChecker(quota.RetryQuota(initial_capacity=5))
        # The first one succeeds.
        self.assertTrue(quota_checker.acquire_retry_quota(self.create_context()))
        # But we should fail now because we're out of quota.
        self.request_context.pop("retry_quota_capacity")
        self.assertFalse(quota_checker.acquire_retry_quota(self.create_context()))
        self.assertNotIn("retry_quota_capacity", self.request_context)

    def test_quota_reached_adds_retry_metadata(self):
        quota_checker = standard.RetryQuotaChecker(quota.RetryQuota(initial_capacity=0))
        context = self.create_context()
        self.assertFalse(quota_checker.acquire_retry_quota(context))
        self.assertEqual(context.get_retry_metadata(), {"RetryQuotaReached": True})
...
class TestRetryContext(unittest.TestCase):
    ...
    def test_can_add_retry_reason(self):
        context = arbitrary_retry_context()
        context.add_retry_metadata(MaxAttemptsReached=True)
        self.assertEqual(context.get_retry_metadata(), {"MaxAttemptsReached": True})
...
class TestThrottlingErrorDetector(unittest.TestCase):
    ...
    def test_no_throttling_error(self):
        ...
        self.assertFalse(self.throttling_detector.is_throttling_error(**kwargs))

    def test_detects_modeled_errors(self):
        kwargs = self.create_needs_retry_kwargs()
        kwargs["response"] = (None, {"Error": {"Code": "ModeledThrottlingError"}})
