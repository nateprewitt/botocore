...
class TestAssumeRoleCredentials(BaseEnvVar):
    ...
    def create_role(self, policy_document, policy_arn=None):
        ...
        if policy_arn:
            ...
            self.addCleanup(
                self.iam.detach_role_policy, RoleName=name, PolicyArn=policy_arn
            )
    ...
    def create_user(self, policy_arns):
        ...
        for arn in policy_arns:
            ...
            self.addCleanup(self.iam.detach_user_policy, UserName=name, PolicyArn=arn)
    ...
    def create_assume_policy(self, role_arn):
        policy_document = {
            "Version": "2012-10-17",
            "Statement": [
                {"Effect": "Allow", "Resource": role_arn, "Action": "sts:AssumeRole"}
            ],
        }
        name = self.random_name()
        ...
        self.addCleanup(self.iam.delete_policy, PolicyArn=response["Policy"]["Arn"])
