...
class TestAcceptedDateTimeFormats(unittest.TestCase):
    ...
    def test_accepts_datetime_object(self):
        response = self.client.list_clusters(CreatedAfter=datetime.datetime.now())
    ...
    def test_accepts_iso_8601_unaware(self):
        response = self.client.list_clusters(CreatedAfter="2014-01-01T00:00:00")
        self.assertIn("Clusters", response)

    def test_accepts_iso_8601_utc(self):
        response = self.client.list_clusters(CreatedAfter="2014-01-01T00:00:00Z")
        self.assertIn("Clusters", response)

    def test_accepts_iso_8701_local(self):
        response = self.client.list_clusters(CreatedAfter="2014-01-01T00:00:00-08:00")
        self.assertIn("Clusters", response)
...
class TestClientErrors(unittest.TestCase):
    ...
    def test_region_mentioned_in_invalid_region(self):
        client = self.session.create_client("cloudformation", region_name="us-east-999")
    ...
    def test_client_modeled_exception(self):
        client = self.session.create_client("dynamodb", region_name="us-west-2")
    ...
    def test_can_catch_client_exceptions_across_two_different_clients(self):
        client = self.session.create_client("dynamodb", region_name="us-west-2")
        client2 = self.session.create_client("dynamodb", region_name="us-west-2")
