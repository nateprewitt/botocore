...
def http_post(url, data, files):
    ...
    response = http.request("POST", url, fields=merged_data,)
    return response
...
class BaseS3ClientTest(unittest.TestCase):
    ...
    def assert_status_code(self, response, status_code):
        self.assertEqual(response["ResponseMetadata"]["HTTPStatusCode"], status_code)
    ...
    def create_object(self, key_name, body="foo", num_attempts=3):
        for _ in range(num_attempts):
            try:
                self.client.put_object(Bucket=self.bucket_name, Key=key_name, Body=body)
                break
...
class TestS3BaseWithBucket(BaseS3ClientTest):
    ...
    def assert_num_uploads_found(
        ...
        for _ in range(num_attempts):
            pages = paginator.paginate(
                Bucket=self.bucket_name, PaginationConfig={"MaxItems": max_items}
            )
    ...
    def create_client(self):
        ...
        return self.session.create_client("s3", self.region, config=client_config)

    def assert_can_put_object(self, body):
        client = self.create_client()
        response = client.put_object(Bucket=self.bucket_name, Key="foo", Body=body)
        self.assert_status_code(response, 200)
...
class TestS3Buckets(TestS3BaseWithBucket):
    ...
    def test_can_make_request(self):
        ...
        self.assertEqual(
            sorted(list(result.keys())), ["Buckets", "Owner", "ResponseMetadata"]
        )
...
class TestS3Objects(TestS3BaseWithBucket):
    ...
    def test_can_delete_urlencoded_object(self):
        key_name = "a+b/foo"
        self.create_object(key_name=key_name)
        bucket_contents = self.client.list_objects(Bucket=self.bucket_name)["Contents"]
        ...
        response = self.client.delete_object(Bucket=self.bucket_name, Key=key_name)
        self.assert_status_code(response, 204)
    ...
    def test_get_object_stream_wrapper(self):
        self.create_object("foobarbaz", body="body contents")
        response = self.client.get_object(Bucket=self.bucket_name, Key="foobarbaz")
        body = response["Body"]
    ...
    def test_unicode_system_character(self):
        ...
        parsed = self.client.list_objects(Bucket=self.bucket_name, EncodingType="url")
        self.assertEqual(len(parsed["Contents"]), 1)
...
class TestS3PresignUsStandard(BaseS3PresignTest):
    def setUp(self):
        ...
        self.client_config = Config(region_name=self.region, signature_version="s3")
    ...
    def test_presign_sigv2(self):
        ...
        self.assertTrue(
            presigned_url.startswith(
                "https://%s.s3.amazonaws.com/%s" % (self.bucket_name, self.key)
            ),
...
class TestS3SigV4Client(BaseS3ClientTest):
    ...
    def test_paginate_list_objects_unicode(self):
        ...
        for key in key_names:
            response = self.client.put_object(Bucket=self.bucket_name, Key=key, Body="")
            self.assert_status_code(response, 200)
    ...
    def test_bad_request_on_invalid_credentials(self):
        ...
        self.client.put_object(Bucket=self.bucket_name, Key="foo.txt", Body=b"asdfasdf")
...
class TestS3UTF8Headers(BaseS3ClientTest):
    def test_can_set_utf_8_headers(self):
        ...
        self.assert_status_code(response, 200)
        self.addCleanup(self.client.delete_object, Bucket=bucket_name, Key="foo.txt")
...
class TestRegionRedirect(BaseS3ClientTest):
    ...
    def test_region_redirect_sigv2_to_sigv4_raises_error(self):
        ...
        sigv2_client = self.session.create_client(
            "s3", region_name=self.client_region, config=Config(signature_version="s3")
        )
    ...
    def test_redirects_head_object(self):
        key = "foo"
        self.bucket_client.put_object(Bucket=self.bucket_name, Key=key, Body="bar")
